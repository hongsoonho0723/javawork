...
public class StudentServiceImpl implements StudentService {
	...
	public List<Student> sortByAge() {
		...
		//자료구조 정렬 - Student객체는 반드시 Comparable를 구현
		//Collections.sort(sortList);
		
		//////Comparator 사용하기///////////////////////////////////////
		/*
		 * Student객체에 Comparable 구현은 할 필요없다.
		 * */
		//Collections.sort(sortList ,new SortCompartorTest());
		
		Collections.sort(sortList , new Comparator<Student>() {
			@Override
			public int compare(Student o1, Student o2) {
				return  o1.getAge() - o2.getAge();//올림차순
			}
		} );
	...
	@Override
	public List<Student> sortDesendingByAge() {
		//정렬한 복사본 list 만들자.
		List<Student> sortList = new ArrayList<Student>(list);
		Collections.sort(sortList);// Student의 채Comparable구현
